    ...
    extends QAgent
{
    ...
    public Model initQFunction()
    {
        ...
        // final int numPixelsInImage = Board.NUM_ROWS * Board.NUM_COLS + 7;
        ...
        final int numInputs = 228; // 7 features (flatness, stackHeight, etc.)
        ...
        // final int inputLength = 7;
        // final int hiddenDim = 2 * inputLength;
        // final int outDim = 1;

        // // Create a feedforward neural network
        // Sequential qFunction = new Sequential();
        // qFunction.add(new Dense(inputLength, hiddenDim)); // First hidden layer
        // qFunction.add(new Tanh()); // Activation function
        // qFunction.add(new Dense(hiddenDim, outDim)); // Output layer

        // return qFunction;


        // Fourth
        // final int inputLength = 227;
        // // final int hiddenDim1 = 2 * inputLength; // First hidden layer size
        // final int hiddenDim1 = 128;
        // // final int hiddenDim2 = inputLength; // Second hidden layer size
        // final int hiddenDim2 = 64;
        // final int outDim = 1; // Output layer size (Q-value as scalar output)

        // // Create a feedforward neural network
        // Sequential qFunction = new Sequential();
        // qFunction.add(new Dense(inputLength, hiddenDim1)); // First hidden layer
        // qFunction.add(new Tanh()); // Activation function
        // qFunction.add(new Dense(hiddenDim1, hiddenDim2)); // Second hidden layer
        // qFunction.add(new Tanh()); // Activation function
        // qFunction.add(new Dense(hiddenDim2, outDim)); // Output layer
                                    ...
                                    final Mino potentialAction)
    {
        ...
        Matrix input = Matrix.full(1, numBoardFeatures + 8, 0);

        // Add board pixels
        // for (int row = 0; row < NUM_ROWS; row++) {
        //     for (int col = 0; col < NUM_COLS; col++) {
        //         input.set(0, row * NUM_COLS + col, boardImage.get(row, col));
        //     }
        // }

        // for (int i = 0; i < numBoardFeatures; i++) {
        //     input.set(0, i, boardImage.get(0, i));
        // }

        // Matrix input = Matrix.full(1, 7, 0);
        ...
        // Non-normalized
        // input.set(0, index, stackHeight);
        // input.set(0, index++, maxHeight);
        // input.set(0, index++, numberOfHoles);
        // input.set(0, index++, colDiff);
        // // input.set(0, index++, sumOfDifferencesWithAvg);
        // input.set(0, index++, actionClear);
        // input.set(0, index++, flatness);
        input.set(0, index++, wellDepth);
                                 ...
                                 final GameCounter gameCounter)
    {
        ...
        // if (scoredPoint) {
        //     boolean random = this.getRandom().nextDouble() <= 0.005;
        //     if (random) {
        //         scoredPoint = false;
        //         return true;
        //     } else {a
        ...
        // if (explorationProb < 0.1) {
        //     return this.getRandom().nextDouble() < 0.1;
        // }
        ...
        // Version 2
        // double decayFactor = 0.99; // Exponential decay
        // double explorationProb = EXPLORATION_PROB * Math.pow(decayFactor, gameCounter.getCurrentGameIdx());
        // explorationProb = Math.max(0.2, explorationProb); // Minimum exploration probability
        // return this.random.nextDouble() < explorationProb;


        // Version 3
        // int currentPhaseIndex = (int) gameCounter.getCurrentPhaseIdx();
        // int currentGameIndex = (int) gameCounter.getCurrentGameIdx();
        // int currentMoveIndex = (int) gameCounter.getCurrentMoveIdx();

        // // Check if the phase has changed
        // if (currentPhaseIndex != previousPhaseIndex) {
        //     // Reset tracking for a new phase
        //     previousMoveCount = 0;
        //     previousGameIndex = 0;
        //     previousPhaseIndex = currentPhaseIndex;
        //     return false; // No exploration at the start of a new phase
        // }

        // // Check if the current game has changed
        // if (currentGameIndex != previousGameIndex) {
        //     // Update min and max move counts for the sampling window
        //     if (previousMoveCount < minMoveCount) {
        //         minMoveCount = previousMoveCount;
        //     }
        //     if (previousMoveCount > maxMoveCount) {
        //         maxMoveCount = previousMoveCount;
        //     }
        // }

        // // Evaluate exploration after every SAMPLE_SIZE games
        // if (currentGameIndex % SAMPLE_SIZE == 0) {
        //     // If the difference between max and min moves exceeds the threshold
        //     if ((maxMoveCount - minMoveCount) > MOVE_DIFF_THRESHOLD && !explorationTriggered) {
        //         explorationTriggered = true; // Trigger exploration
        //         return true; // Explore due to being "stuck"
        //     }
        //     // Reset for the next sampling window
        //     minMoveCount = Integer.MAX_VALUE;
        //     maxMoveCount = Integer.MIN_VALUE;
        // } else {
        //     explorationTriggered = false; // Reset exploration trigger
        // }

        // // Update state variables
        // previousMoveCount = currentMoveIndex;
    ...
    public double getReward(final GameView game)
    {
        ...
        // reward = (game.getScoreThisTurn() * 100000);
        // reward -= (10 * numberOfHoles);
        // reward -= (4 * colDiff);
        // reward -= (7 * sumOfDifferencesWithAvg);
        // if (stackHeight > 4) {
        //     reward -= (Math.pow(stackHeight, 2) * 5);
        // }
        // reward -= (10 * maxHeight);
        // reward += (100000 * actionClear);
        // reward += (flatness);
        ...
        // reward += 5.0 * actionClear; // Reward for clearing lines
        // reward += 2.0 * flatness; // Reward for achieving flatness


        // reward += 0.7 * game.getScoreThisTurn();
        reward += 100.0 * game.getScoreThisTurn();
        ...
        reward += 3 * flatness; // Reward for achieving flatness

        reward -= 0.5 * setup;
        reward -= 0.3 * colHeight;
        reward -= 0.2 * depth;
        ...
        // reward -= Math.exp(stackHeight / NUM_ROWS);
        reward -= 0.3 * wellDepth;

        
        // reward -= 0.1 * aggregateHeight; // Penalize high aggregate height
        // reward -= 0.2 * bumpiness; // Penalize uneven columns
        // reward -= 0.2 * overhangs; // Penalize overhangs
        

        // Additional penalty for losing (top row occupied)
        // for (int col = 0; col < NUM_COLS; col++) {
        //     if (board.isCoordinateOccupied(col, 0)) {
        //         reward -= 10.0; // Large penalty for losing
        //     }
        // }

        // Limiter for the max we can decrease
